...
\begin{tcolorbox}[colback=white!5!white, colframe=lightGrey, boxrule=0.3mm, arc=0mm, boxsep=0mm, left=1mm, right=1mm, bottom=1mm, top=1mm]
	\begin{tcolorbox}[colback=backgroundGray, colframe=white, arc=0mm, fontupper=\color{textColor}\sffamily\bfseries]
		\large Abstracto\\\\
		\normalsize El objetivo del presente documento es el de brindar la información al usuario de cómo crear en el sistema MES un conjunto de seriales personalizados. El cliente utilizado y donde se hará la configuración y pruebas será TORO.  
	\end{tcolorbox}%
\end{tcolorbox}

\section{Creación de seriales}
Ejemplo de creación de seriales personalizados en MES STG.
Ingresamos al servidor de ejemplo \texttt{MXGDLM0MESSTG81} y abrimos la aplicación de MES. El serializado se configura en \texttt{Labels - Serials}  $\rightarrow$ \texttt{Assembly Serialization}.
...
\begin{figure}[H]
	...
	\caption{Assembly Serialization.}
	\label{fig:pic1}
\end{figure}

El sistema MES nos permite colocar constantes, datecodes e incrementos para formar la estructura de un nuevo serial. Para agregar un nuevo tipo de serializado, damos clic sobre el símbolo de la carpeta que se muestra en la parte inferior izquierda de la pantalla de MES, esta acción nos habilitará todos los campos que estaban por defecto inhabilitados.

\begin{figure}[H]
	\centering
	\includegraphics[scale = 1]{Picture2}
	\caption{Creando nuevo serializado}
	\label{fig:pic2}
\end{figure}

El primer paso es establecer un nombre para el nuevo serializado a crear.
\begin{figure}[H]
	\centering
	\includegraphics[scale = 1]{Picture3}
	\caption{Colocando un nombre para el serializado}
	\label{fig:pic2}
\end{figure}
