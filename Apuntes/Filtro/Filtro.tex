...
\noindent Sin embargo, si la magnitud del voltaje $v_S$ cae por debajo del voltaje instantáneo del capacitor C los diodos ($D_1$ y $D_2$ o $D_3$ y $D_4$) estarán polarizados en inversa y el capacitor C se descargará a través de la resistencia de carga $R_L$. El voltaje del capacitor $v_C$ variará entre un valor mínimo $V_{o(min)}$ y un valor máximo $V_{o(max)}$. Las formas de onda del voltaje de salida $v_O$ y el voltaje de rizado $v_r$ se muestran en la figura 2. Si $f$ es la frecuencia de la fuente de entrada, entonces el periodo del voltaje de entrada es $T=1/f$.
	...
	\begin{tikzpicture}[scale=0.76, remember picture]
		...
		\begin{axis}[
			...
			\draw[<-](pi/2, 3.05)--(pi/2 + 0.4, 3.3); %vo_max
			\node[above] at (axis cs:1.970796,3.3){$V_{o(max)}$};
			\draw[<-](2.28*pi, 2.5)--(2.1*pi, 2.8); %vo_min
			\node[above] at (axis cs:2.1*pi, 2.8){$V_{o(min)}$};
			\draw[->|] (0.1*pi,1) -- (0.3*pi,1) node[right=0.9mm]{{\Large $t_c$}};
			\draw[<-] (0.5*pi,1) -- (1.2*pi,1);
			\draw[->|] (1.6*pi,1) node[left=3mm]{{\Large $t_d$}} -- (2.3*pi,1);
			\draw[|<-] (0.3*pi,0.6) -- (1.1*pi,0.6) node[right=3.5mm]{{\Large $T$}};
			\draw[->|] (1.5*pi,0.6) -- (2.3*pi,0.6);
			\draw[|<-] (0.5*pi,0.2) -- (1.3*pi,0.2) node[right=2mm]{{\large $\beta$}};
			\draw[->|] (1.5*pi,0.2) -- (2.3*pi,0.2);
		\end{axis}
		...
		\begin{axis}[
			...
			\addplot[mark=none, blue, domain=0:0.3*pi] {((-0.4-0.4)/(0.3*pi +0.5*pi))*(x + 0.5*pi) + 0.4};
			\addplot[mark=none, blue, domain=0.3*pi:0.5*pi] {((0.4+0.4)/(0.5*pi - 0.3*pi))*(x - 0.3*pi) - 0.4};
			\addplot[mark=none, blue, domain=0.5*pi:2.3*pi] {-0.141471*(x - 0.5*pi) + 0.4};
			\addplot[mark=none, blue, domain=2.3*pi:2.5*pi] {((0.4+0.4)/(0.5*pi - 0.3*pi))*(x - 0.3*pi - 2*pi) - 0.4};
			\addplot[mark=none, blue, domain=2.5*pi:3*pi] {-0.141471*(x - 0.5*pi - 2*pi) + 0.4};
			\node[above] at (axis cs:0,1.5){$v_r$};
			\draw[|<->|] (0.5*pi + 0.5,0.4) node[above]{$V_{r(pp)}$} -- (0.5*pi + 0.5,-0.4) ;
		\end{axis}
	\end{tikzpicture}
...
\noindent Las ecuaciones en (15) pueden usarse para encontrar el valor del capacitor C con una precisión razonable para la mayoría de de los propósitos prácticos siempre y cuendo el factor de rizo se encuentre dentro del 10\%. Podemos observar de la ecuación (15) que para la misma cantidad de voltaje de rizado , el rectificador de onda completa requerirá la mitad de la capacitancia $C$ debido a la doble frecuencia de rizado $2f$ a comparación con el rectificador de media onda. \\

\noindent Si asumimos que el voltaje de salida decrece linealmente desde $V_{o(max)}(=V_m)$ hasta $V_{o(min)}$ durante el intervalo de descarga, el voltaje de salida promedio puede ser encontrado de forma aproximada de la siguiente manera:

\begin{align}
	V_{o(avg)} = \frac{V_m + V_{o(min)}}{2} = \frac{1}{2} \left[ V_m + V_m \left( 1 - \frac{t_d}{R_L C} \right) \right]
\end{align} 

Al sustituir la ecuación (13) en (16) obtenemos

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,arc=0mm,boxrule=0mm,width=(\linewidth+0.5cm)]
	\begin{equation}
		V_{o(avg)} =   
		\begin{cases}
			\frac{1}{2} \left[ V_m + V_m \left( 1 - \frac{1}{2R_L fC} \right) \right]= \frac{V_m}{2} \left[ 2 - \frac{1}{2R_L fC} \right] \quad & rectificador \,\, de \,\, onda \,\, completa\\\\va
			\frac{1}{2} \left[ V_m + V_m \left( 1 - \frac{1}{R_L fC} \right) \right]= \frac{V_m}{2} \left[ 2 - \frac{1}{R_L fC} \right] \quad & rectificador \,\, de \,\, media \,\, onda
		\end{cases}
	\end{equation}
\end{tcolorbox}

\section*{Ejemplo de Diseño}
Un rectificador de onda completa monofásico se alimneta directamente desde una fuente de $\qty{120}{\volt}$ a $\qty{60}{\hertz}$ sin ningún transformador de entrada. La resistencia de carga es $R_L = \qty{500}{\ohm}$
\begin{enumerate}[label=\alph*)]
	\item Diseñe un filtro C de tal manera que el voltaje pico a pico se encuentre dentro del 10\% de $V_m$
	\item Con el valor de $C$ encontrado en (a), calcule el voltaje de salida $V_{o(avg)}$ y el voltaje en el capacitor si la carga se encuentra desconectada
\end{enumerate}

\subsection*{Solución}
\noindent (a)
\begin{align}
	V_m = \sqrt{2}V_s = \sqrt{2} \times 120 = \qty{169.7}{\volt} \nonumber
\end{align}

\noindent El voltaje de rizado pico a pico es

\begin{align}
	V_{r(pp)} = 10\% V_m = 0.1 \times \qty{169.7}{\volt} = \qty{16.97}{\volt} \nonumber
\end{align}

\noindent El voltaje de salida mínimo es

\begin{align}
	V_{o(min)} = V_m  - V_{r(pp)} = 170 - 16.97 = \qty{152.74}{\volt} \nonumber
\end{align}

\noindent Obtenemos el ángulo $\alpha$ a partir de la ecuación (2)
\begin{align}
	\alpha = \arcsin \left( \frac{V_{o(min)}}{V_m} \right) = \qty{1.12}{\radian} = \qty{64.16}{\degree} \nonumber
\end{align}

\noindent Obtenemos el ángulo de descarga a partir de la ecuación (8)

\begin{align}
	\beta = \frac{\pi}{2} + \alpha = \frac{\pi}{2} + 1.12 = \qty{2.698}{\radian} = \qty{154.16}{\degree} \nonumber
\end{align}

\noindent Obtenemos el valor del capacitor a partir de la ecuación (12)

\begin{align}
	C = \frac{ \frac{\pi}{2}  + \alpha }{ 2\pi f R_L \ln \left( \frac{V_m}{V_{o(min)}} \right) } = \frac{ \frac{\pi}{2} + 1.12 }{2\pi \times 86 \times 500 \times \ln (169.7 / 152.74)} = \qty{135.48}{\micro \farad} \nonumber
\end{align}

\noindent La ecuación aproximada para $C$ usando la ecuación (15) nos da un valor de :

\begin{align}
	C = \frac{V_m}{2fR_L V_{r(pp)}} = \frac{169.7}{2 \times 60 \times 500 \times 16.97} = \qty{166.7}{\micro \farad} \nonumber
\end{align}

\noindent Utilizamos la ecuación (17) para obtener el voltaje promedio

\begin{align}
	V_{o(avg)} = \frac{V_m}{2} \left[ 2 - \frac{1}{2R_L f C} \right] = \frac{169.7}{2} \left[ 2 - \frac{1}{2 \times 500 \times 60 \times 166.7 \times 10^{-6}} \right] = \qty{161.21}{\volt} \nonumber
\end{align}

\noindent Si la reistencia $R_L$ se desconecta, el capacitor se cargará hasta el calor pico del voltaje de entrada $V_m$. por lo tanto, el voltaje de salida promedio con la carga desconectada es de

\begin{align}
	V_{o (no \, load)} = V_m = \qty{169.7}{\volt}
\end{align}

\noindent El voltaje de salida promedio $V_{o(avg)}$ = cambiará desde $\qty{169.7}{\volt}$ a $\qty{161.2}{\volt}$ si se conecta la carga. Este cambio en el voltaje se le conoce como \textit{regulación de voltaje} el cual se define como

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,arc=0mm,boxrule=0mm,width=(\linewidth+0.5cm)]
	\begin{equation}
		\texttt{Regulación de voltaje} = \frac{V_{0(no-load)} - V_{o(load)}}{V_{o(load)}} = \frac{V_{o(load)} - V_{o(avg)}}{V_{o(avg)}}
	\end{equation}
\end{tcolorbox}
...
\begin{align}
	\texttt{Regulación de voltaje} = \frac{169.7 - 161.2}{161.2} = 5.27\% \nonumber
\end{align}
\end{document}