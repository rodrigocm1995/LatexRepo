...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.8]
		...
		\draw (0,0) to[V] ++(0,6)
		to[short] ++(4,0)
		to[R,-*, l=$R_1$] ++(0,-3) coordinate(p1)
		to[R=$R_2$,-*] ++(0,-3) coordinate(p2)
		to[short] ++(-4,0); 
		\draw(p1) to[short] ++(3,0)
		to[short,-*] ++(0,-0.65) coordinate(m1);
		%\draw[black, thick] (6,2.5) node[right=8mm,below=2mm]{Load} rectangle (8,0.5);
		\draw(p2) to[short] ++(3,0)
		to[short,-*] ++(0,0.65);
		\node[draw, align=center] at (7,1.5) {Load\\ +3V \\ 9.1 mA};
	\end{tikzpicture}
