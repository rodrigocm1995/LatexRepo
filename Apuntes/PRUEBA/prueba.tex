...
\begin{document}
	...
	Realice en análisis de cd y ac para el circuito de la figura 1.
		...
		\begin{tikzpicture}[scale=0.8]
			...
			\draw (Q1.B) to[R, l=\SI{1}{\kilo\ohm}, a=$R_B$, -o] ++(-4,0) node[left]{input};
			\draw (Q1.C) to[short, -*] ++(0,1) coordinate (n1);
			\draw (n1) to[R, l_=Load] ++(0,6) coordinate(n3) node[vcc]{$V_{CC}=\SI{12}{\volt}$};
			\draw (n1) to[short] ++(-2,0)
						to[leD*, invert, l=LED1] ++(0,3)
						to[R, l2=$R_{1}$ and \SI{500}{\ohm}] ++(0,3) coordinate (G) 
						to[short, -*] ++(2,0) coordinate(F);
			\draw (n1) to[short] ++(2,0) coordinate (n2);
			\draw (n2) node[npn, right](Q2){$Q_2$};
			\draw (Q2.E) to[R, l2=$R_{2}$ and \SI{500}{\ohm}] ++(0,-3)
						to[leD*, l=LED2] ++(0,-3) coordinate(n4) node[ground]{};
			\coordinate(C) at (Q2.C |- n3);
			\coordinate(D) at (Q1.E |- n4);
			\draw (Q2.C) to[short] (C) coordinate(E);
			\draw (Q1.E) to[short] (D) node[ground]{};
			\draw (F) to[short] (E);
		\end{tikzpicture}
